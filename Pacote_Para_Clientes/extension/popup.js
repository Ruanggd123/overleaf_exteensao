...
function checkServerConnection() {
    ...
    fetch(`${API_BASE_URL}/status`, { signal: controller.signal })
        ...
        .then(data => {
            ...
            // Enable login checks
            if (auth.currentUser) loadUserData();
        })
        .catch(error => {
...
const db = firebase.database();  // Primary source for status
const firestore = firebase.firestore(); // Keep for specific queries if needed
...
async function loadUserData() {
    ...
    try {
        ...
        chrome.storage.local.get(['isActive'], (result) => {
            ...
            if (toggle) toggle.checked = result.isActive !== false; // Default true
        });
...
function updateDashboardFromRealtime(data) {
    ...
    if (subscription.expiresAt) {
        ...
        // Update some UI element with days remaining if you have it, or reusing usage-text
        document.getElementById('usage-text').textContent = `${diffDays} dias restantes`;
        // usage bar full if active
        document.getElementById('usage-bar').style.width = '100%';
    }

    // Hide buy buttons if pro
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // document.getElementById('btn-email-anon').addEventListener('click', signInAnonymous); // Removed
